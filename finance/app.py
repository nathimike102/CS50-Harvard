...
@login_required
def index():
    totalBalance = balance = db.execute("SELECT cash FROM users WHERE id = ?", session["user_id"])[0]["cash"]
...
def buy():
    if request.method == "POST":
        ...
        if new_balance < 0:
            ...
            db.execute("INSERT INTO transactions (symbol, quantity, price, u_id) VALUES(?, ?, ?, ?)",request.form.get("symbol"), float(request.form.get("shares")), price, session["user_id"])
            return redirect("/")
...
def history():
    ...
    portfolio = db.execute("SELECT t_id, symbol, quantity, price, u_id FROM transactions WHERE u_id=? ORDER BY t_id DESC", session["user_id"])
...
def quote():
    """Get stock quote."""
    return apology("TODO")    submitted = False
    if request.method == "POST":
        submitted = True
