...
void card_type(long card)
{
    ...
    else if (card / 10000000000000 == 34 ||
             card / 10000000000000 == 37) // Check if it's a AMERICAN EXPRESS
    {
        printf("AMEX\n");
    }
    else if (card / 100000000000000 >= 51 &&
             card / 100000000000000 <= 55) // Check if it's a MASTERCARD
    {
        printf("MASTERCARD\n");
    }
    else
...
bool check_sum(long card)
{
    ...
    int d16 =
        (((card / 1000000000000000) % 10) * 2 / 10) + (((card / 1000000000000000 % 10) * 2) % 10);
    ...
    odds = d1 + d3 + d5 + d7 + d9 + d11 + d13 + d15;
    int sum = pairs + odds;
